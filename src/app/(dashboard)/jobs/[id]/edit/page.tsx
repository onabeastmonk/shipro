      ...
      <div className="p-4 space-y-4">
        ...
        <Section title="RATES">
          <div className="grid grid-cols-2 gap-2 mb-3">
            <button type="button" onClick={() => update('pricing_mode', 'fixed')}
              className="p-2.5 rounded-lg border text-left"
              style={{ border: form.pricing_mode !== 'per_cbm' ? '2px solid #60a5fa' : '1px solid rgba(255,255,255,0.08)', background: form.pricing_mode !== 'per_cbm' ? 'rgba(96,165,250,0.1)' : 'transparent' }}>
              <div className="text-xs font-bold" style={{ color: form.pricing_mode !== 'per_cbm' ? '#60a5fa' : '#a0a0a0' }}>Fixed Rate</div>
            </button>
            <button type="button" onClick={() => update('pricing_mode', 'per_cbm')}
              className="p-2.5 rounded-lg border text-left"
              style={{ border: form.pricing_mode === 'per_cbm' ? '2px solid #22c55e' : '1px solid rgba(255,255,255,0.08)', background: form.pricing_mode === 'per_cbm' ? 'rgba(34,197,94,0.1)' : 'transparent' }}>
              <div className="text-xs font-bold" style={{ color: form.pricing_mode === 'per_cbm' ? '#22c55e' : '#a0a0a0' }}>Per CBM</div>
            </button>
          </div>
          <div className="grid grid-cols-2 gap-3">
            {form.pricing_mode === 'per_cbm' ? (
              <Field label="Rate per CBM (₱/CBM)">
                <input className="form-input" type="number" step="0.01" placeholder="e.g. 500" value={form.rate_per_cbm} onChange={e => update('rate_per_cbm', e.target.value)} />
              </Field>
            ) : (
